// === Practice 1 ===

//1. Creati o clasa Person care sa aiba firstName si lastName in constructor
//2. aduagati o metoda displayPersonName care sa returneze numele complet
//3. creati o persoana cu noua clasa unde dati un nume si-un prenume si apelati metoda respectiva


//cerinta 1 - Creati o clasa Person care sa aiba firstName si lastName in constructor

class Person {
  firstName;
  lastName;
  
  constructor(firstName, lastName) {
    this.firstName = firstName;
    this.lastName = lastName;
  }
  
//cerinta 2 - aduagati o metoda displayPersonName care sa returneze numele complet

  displayPersonName() {
    console.log(`${this.firstName} ${this.lastName}`);
  }
}

//cerinta 3 - creati o persoana cu noua clasa unde dati un nume si-un prenume si apelati metoda respectiva

let newPerson = new Person("Jane", "Doe");
newPerson.displayPersonName();


console.log("-------------------------------") // am adaugat aceasta linie pentru a-mi fi mai usor sa-mi dau seama unde se incheie un exercitiu

// === Practice 2 ===

//1. Creati o clasa Animal care sa aiba in constructor name si weight

//2. Creati urmatoarele metode: eatFood, sleep, wakingUp - fiecare metoda o sa returneze un string '$name is eating' '$name is sleeping' '$name is weaking up'

//3. Creati o clasa noua care sa faca extend la clasa Animal si sa ia numele si weight-ul de la el(tip: super) - ex: class Elephant, class Lion etc

//4. Adaugati 4 metode noi:
//   walking - $name is walking
//   rage - $name is rageing 
//   showBehaviour - apelati metodele eatFood si rage
//   dailyRoutine - apelati metodele wakingUp, rage, eatFood si sleep

//5. Creati o constanta noua din clasa cu numele de animal pe care ati scris-o mai sus( NU DIN clasa Animal ci din clasa Elephant, Monkey, Lion etc etc);

//6. Accesati toate cele 4 metode de pe noul animal. 

// - Cerinta 1 - Creati o clasa Animal care sa aiba in constructor name si weight

class Animal {
  name;
  weight;

  constructor(name, weight) {
    this.name = name;
    this.weight = weight;
  }

// - Cerinta 2 - Creati urmatoarele metode: eatFood, sleep, wakingUp - fiecare metoda o sa returneze un string '$name is eating' '$name is sleeping' '$name is weaking up'

  eatFood() {
    console.log(`${this.name} ${" is eating"}`);
  }

  sleep() {
    console.log(`${this.name} ${" is sleeping"}`);
  }

  wakingUp() {
    console.log(`${this.name} ${" is waking up"}`);
  }
}

// - Cerinta 3 - Creati o clasa noua care sa faca extend la clasa Animal si sa ia numele si weight-ul de la el(tip: super) - ex: class Elephant, class Lion etc

class Lion extends Animal {
  constructor(name, weight) {
    super(name, weight);
  }

// - Cerinta 4 - Adaugati 4 metode noi: walking - $name is walking; rage - $name is rageing;   showBehaviour - apelati metodele eatFood si rage; dailyRoutine - apelati metodele wakingUp, rage, eatFood si sleep

  walking() {
    console.log(`${this.name} ${" is walking"}`);
  }

  rage() {
    console.log(`${this.name} ${" is rageing"}`);
  }

  showBehaviour() {
    super.eatFood();
    this.rage();
  }

  dailyRoutine() {
    super.wakingUp();
    this.rage();
    super.eatFood();
    super.sleep();
  }
}

// - Cerinta 5 -
//5. Creati o constanta noua din clasa cu numele de animal pe care ati scris-o mai sus( NU DIN clasa Animal ci din clasa Elephant, Monkey, Lion etc etc);

const lioness = new Lion("Nala");

// - Cerinta 6 - Accesati toate cele 4 metode de pe noul animal. 
console.log(lioness.walking()); // aici dupa fiecare afisare corecta a cerintei cu console.log imi apare si undefined - Am scris ceva gresit, mai trebuia adaugat/modificat ceva?
console.log(lioness.rage());
console.log(lioness.showBehaviour());
console.log(lioness.dailyRoutine());


console.log("-------------------------------")


// === Practice 3 ===


//1. creati o clasa Account care sa aiba location si name la constructor

class Account {
  name;
  location;

  constructor(name, location) {
    this.name = name;
    this.location = location;
  }
}

//2. creati o clasa Transaction care sa aiba sender, bank, amount si reference la constructor

class Transaction {
  sender;
  bank;
  amount;
  reference;

  constructor(sender, bank, amount, reference) {
    this.sender = sender;
    this.bank = bank;
    this.amount = amount;
    this.reference = reference;
  }

  //2.1 - clasa Transaction sa aiba un getter finalSum() care sa scada din amount 10;

  get finalSum() {
    this.amount -= 10;
    return this.amount;
  }
}


//3. creati o clasa Table care sa aiba in cunstructor un array gol transactions

//const transaction = new Transaction();

class Table {
  transactions;

  constructor(transactions = []) {
    this.transactions = transactions;
  }

  //3.1 Clasa Table sa aiba o metoda addTransaction unde sa putem adauga tranzactiile pe care o sa le creem mai tarziu

  addTransaction(transaction) {
    this.transactions.push(transaction);
    return this.transactions;
  }
}

const bankAccount = new Account ("John Doe", "Iasi");
console.table(bankAccount);

const transaction1 = new Transaction("Jane Doe", "BT", 500, 12345);
console.table(transaction1);

const transaction2 = new Transaction("Dan Popa", "BCR", 200, 12567);
console.table(transaction2)

const table = new Table();
table.addTransaction(transaction1); 
table.addTransaction(transaction2);
console.table(table);

